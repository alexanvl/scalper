...
const /*input*/ int ATR_PERIOD = 20;
input int      ATR_FACTOR_STOP = 3;
input int      ATR_FACTOR_BOUNCE = 3;
...
double GetBounceSignal()
{
   ...
   double bounceSpread = NormalizeDouble((iATR(NULL,0,ATR_PERIOD,1)*ATR_FACTOR_BOUNCE), Digits);
   
   if (distance < bounceSpread) {
     m_signalBounce = 0;
...
void CheckForOpen()
{
   ...
   double limit = 0;
   double stop = 0;
   double atrLevel = NormalizeDouble((iATR(NULL,0,ATR_PERIOD,1)*ATR_FACTOR_STOP), Digits);
